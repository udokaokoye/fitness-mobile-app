...
const Fasting = () => {
  ...
  function displayTime() {
    ...
    settimeCompletedInPercentage(calculateTimePercentage(fastingContext?.fastingInfo?.startTime, fastingContext?.fastingInfo.endTime))
  }
          ...
          >
            <View className=" justify-center items-center">
              ...
              <FastingCircle
                ...
                size={1000}
                progressTitle={isFasting ? "Elapsed Time" : "Time Since Last Fast"}
...
{isFasting && (
  <View style={{backgroundColor: theme.background2}} className="flex-row justify-between mt-3 mb-5 p-5 rounded-2xl">
  <View>
              <Text style={{color: theme.text}} className="text-center font-bold">STARTED</Text>
              <Text style={{color: theme.accentColor}} className="font-bold text-lg">Today, {moment(fastingContext.fastingInfo.startTime).format("hh:mm a")}</Text>
            </View>
            <View>
              <Text style={{color: theme.text}} className="text-center font-bold">GOAL</Text>
              <Text style={{color: theme.accentColor}} className="font-bold text-lg">{moment(fastingContext.fastingInfo.startTime).format("D MMM") == moment(fastingContext.fastingInfo.endTime).format("D MMM") ? 'Today' : moment(fastingContext.fastingInfo.endTime).format("MMM D,")}, {moment(fastingContext.fastingInfo.endTime).format("hh:mm a")}</Text>
            </View>
  </View>
)}

          <TouchableOpacity onPress={toggleInterval} style={{ backgroundColor: theme.accentColor }} className="rounded-3xl w-60 h-12 mb-10 justify-center items-center self-center" >
            <Text style={{ color: 'white', fontWeight: 'bold' }}>{isFasting ? "Stop Fasting" : "Start Fasting"}</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
    </SafeAreaView>
